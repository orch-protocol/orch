...
func main() {
	...
	// ORCH_ADDR should be "hostname:port"
	...
	go func() {
		for {
			...
			case <-ctx.Done():
				return
			case <-time.After(2 * time.Second): // Slightly faster for Docker demo
			}
		}
	}()
	...
	go func() {
		log.Printf("Load Balancer starting on %s (Orch: %s)", server.Addr, orchAddr)
